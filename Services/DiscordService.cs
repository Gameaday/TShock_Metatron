...
public class DiscordService
{
    ...
    // 🛡️ SECURITY: Event to delegate disconnects to the main thread and avoid cross-thread exceptions
                ...
                if (_auditQueue.TryDequeue(out string? accountName) && _db.Ledger.TryGetValue(accountName, out var record))
                {
                    ...
                    if (!isStillValid)
                    {
                        ...
                        if (_db.Ledger.TryRemove(accountName, out _))
                        {
                            ...
                            // 🛡️ SECURITY: Emit event to enqueue disconnect safely on the main thread
