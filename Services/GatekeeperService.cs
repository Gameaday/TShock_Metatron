        ...
        if (args.MsgID == PacketTypes.PasswordSend)
        {
            ...
            if (isPinGuess)
            {
                ...
                if (strikeData.Strikes >= 5)
                {
                    ...
                    {
                        var cp = TShock.Players[pIndex];
                        if (cp != null && cp.Active && cp.Name == pName && cp.UUID == pUuid)
                            cp.Disconnect("Disconnected: Too many invalid PIN attempts. Please wait 15 minutes before trying again.");
                    });
                    args.Handled = true;
                    return;
                }
            ...
            if (_discord.PendingPins.TryGetValue(entered, out var data))
            {
                if (DateTime.UtcNow > data.Expiry)
                {
                    ...
                    if (isPinGuess)
                    {
                        ...
                        _mainThreadActions.Enqueue(() =>
                        {
                            var cp = TShock.Players[pIndex];
                            if (cp != null && cp.Active && cp.Name == pName && cp.UUID == pUuid)
                                cp.SendErrorMessage($"Invalid PIN. Attempts remaining: {5 - newStrikes}");
                        });
                    }
                    args.Handled = true;
                    return;
                ...
                if (_db.Ledger.TryGetValue(player.Name.ToLower(), out var record))
                {
                    if (record.DiscordId != data.DiscordId && !player.IsLoggedIn)
                    {
                        ...
                        _mainThreadActions.Enqueue(() =>
                        {
                            var cp = TShock.Players[pIndex];
                            if (cp != null && cp.Active && cp.Name == pName && cp.UUID == pUuid)
                                cp.SendErrorMessage("Identity mismatch! This account is pinned to another Discord user. Log in with your recovery password first.");
                        });
                ...
                else if (TShock.UserAccounts.GetUserAccountByName(player.Name) != null && !player.IsLoggedIn)
                {
                    ...
                    _mainThreadActions.Enqueue(() =>
                    {
                        var cp = TShock.Players[pIndex];
                        if (cp != null && cp.Active && cp.Name == pName && cp.UUID == pUuid)
                            cp.SendErrorMessage("This account already exists. Log in with your password first before linking to Discord.");
                    });
                    args.Handled = true; return;
                }
                ...
                _discord.PendingPins.TryRemove(entered, out _);
                args.Handled = true;
                int fIndex = player.Index;
                string fName = player.Name;
                string fUuid = player.UUID;
                _mainThreadActions.Enqueue(() =>
                {
                    var cp = TShock.Players[fIndex];
                    if (cp != null && cp.Active && cp.Name == fName && cp.UUID == fUuid)
                        FinalizeLinkage(cp, data.DiscordId);
                });
            }
            else if (isPinGuess)
            {
