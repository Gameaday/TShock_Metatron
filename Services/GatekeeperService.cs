...
public class GatekeeperService
{
    ...
    public void EnableHooks()
    {
        ...
        _discord.KickRequested += OnDiscordKickRequested;
    }
    ...
    private void OnDiscordKickRequested(string accountName, string reason)
    {
        _mainThreadActions.Enqueue(() =>
        {
            var target = TShock.Players.FirstOrDefault(p => p?.Account?.Name.ToLower() == accountName);
    ...
    private void OnGetData(GetDataEventArgs args)
    {
        ...
        if (args.MsgID == PacketTypes.PasswordSend)
        {
            ...
            if (_discord.PendingPins.TryGetValue(entered, out var data))
            {
                ...
                _discord.PendingPins.TryRemove(entered, out _);
                args.Handled = true; _mainThreadActions.Enqueue(() => FinalizeLinkage(player, data.DiscordId));
            }
            else if (isPinGuess)
            {
                var currentStrikeData = _verifyStrikes.GetOrAdd(ip, (0, now));
