...
public class RestMessageChannel
{
    ...
    {
        var json = await _client.PostJsonAsync($"/channels/{Id}/messages", new { content = text, allowed_mentions = allowedMentions ?? new { parse = Array.Empty<string>() } });
...
public class RestMessage
{
    ...
    public async Task ModifyAsync(string newContent, object? allowedMentions = null) => await _client.PatchJsonAsync($"/channels/{ChannelId}/messages/{Id}", new { content = newContent, allowed_mentions = allowedMentions ?? new { parse = Array.Empty<string>() } });
